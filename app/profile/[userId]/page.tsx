...
const [averageRating, setAverageRating] = useState(0);
  ...
  const handleSendMessage = () => {
    if (swapRequests.length === 0) {
      alert("No active swap found.");
      return;
    }
  
    router.push(`/chat/${swapRequests[0].id}`);
  };
  const handleProposeSwap = async () => {
    ...
    try {
      const result = await sendSwapRequest(currentUser.uid, userId);
        ...
        <div className="bg-white dark:bg-gray-800 shadow overflow-hidden sm:rounded-lg">
            ...
            <div className="border-t border-gray-200 px-4 py-5 sm:px-6">
              ...
              <div className="h-20 w-20 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center">
                  {userProfile?.photoURL ? (
                    ...
                    <span className="text-gray-600 dark:text-white font-medium text-2xl">
              ...
              <dl className="mt-6 grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                <div className="sm:col-span-1">
                  ...
                  <dd className="mt-1 text-sm text-gray-900 dark:text-white">{userProfile?.location || 'Not specified'}</dd>
                </div>
                <div className="sm:col-span-1">
                <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Availability</dt>
                  <dd className="mt-1 text-sm text-gray-900 dark:text-white">{userProfile?.availability || 'Not specified'}</dd>
                </div>
              </dl>
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 px-4 py-5 sm:px-6">
              <div className="grid grid-cols-1 gap-y-6 sm:grid-cols-2 gap-x-6">
                {/* Skills Offered */}
                <div>
                <h3 className="text-lg font-medium text-gray-900 dark:text-white">Skills Offered</h3>
                  <div className="mt-4">
                    {userProfile?.skillsOffered && userProfile.skillsOffered.length > 0 ? (
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400">No skills offered</p>
                    )}
                  </div>
                </div>
                <div className="mt-6 border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-white dark:bg-gray-900">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">User Rating</h2>
              ...
              Total Reviews: {reviews.length}
               ...
               <div className="mt-6 border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-white dark:bg-gray-900">
               <h2 className="text-lg font-semibold mb-4 text-gray-900 dark:text-white">Reviews</h2>
  ...
  {reviews.length > 0 ? (
    ...
    <p className="text-gray-500 dark:text-gray-400">No reviews yet.</p>
                ...
                {/* Skills Wanted */}
                <div>
                <h3 className="text-lg font-medium text-gray-900 dark:text-white">Skills Wanted</h3>
                  <div className="mt-4">
                    {userProfile?.skillsWanted && userProfile.skillsWanted.length > 0 ? (
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400">No skills wanted</p>
                    )}
                  </div>
                </div>
              </div>
            ...
            {swapRequests.length > 0 && (
              <div className="border-t border-gray-200 dark:border-gray-700 px-4 py-5 sm:px-6">
               <h3 className="text-lg font-medium text-gray-900 dark:text-white">Swap History</h3>
                <div className="mt-4">
                  <ul className="divide-y divide-gray-200">
