...
export default function Conversation({ params }: { params: { userId: string } }) {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        if (foundConversation) {
          ...
          const messagesRef = collection(db, 'conversations', foundConversation.id, 'messages');
          const messagesQuery = query(messagesRef, orderBy('timestamp', 'asc'));
          
          const unsubscribeMessages = onSnapshot(messagesQuery, (snapshot) => {
            const messagesData = snapshot.docs.map((docSnapshot: QueryDocumentSnapshot) => {
              const data = docSnapshot.data();
              return {
                id: docSnapshot.id,
                text: data.text || '',
          ...
          // Update unread count
          if (foundConversation.lastSender !== user.uid && foundConversation.unreadCount > 0) {
            await updateDoc(doc(db, 'conversations', foundConversation.id), {
              unreadCount: 0,
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("STATE conversationId:", conversationId);
      ...
      if (!activeConversationId) {
        ...
        console.log("CREATED:", activeConversationId);
  
        setConversationId(activeConversationId);
  
        // ⚠️ IMPORTANT: listener yahan mat rakho (REMOVE THIS)
      }
      ...
      console.log("UPDATED CONVERSATION OK");
